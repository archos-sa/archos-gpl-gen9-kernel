...
struct usbhs_omap_platform_data {
	...
	unsigned			deep_bus_suspend:1;

	/* Regulators for USB PHYs.
	 * Each PHY can have a separate regulator.
	 */
	struct regulator        *regulator[OMAP3_HS_USB_PORTS];
	struct regulator        *regulator_optional[OMAP3_HS_USB_PORTS];

	int	(*platform_bus_suspend) (void);
	int	(*platform_bus_resume) (void);
	int	(*platform_bus_disable) (void);
	int	(*platform_bus_enable) (void);
};
