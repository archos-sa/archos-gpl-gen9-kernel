...
int __init archos_compass_init(struct akm8975_platform_data *pdata)
{
	...
	if ( hardware_rev >= compass_cfg->nrev ) {
		printk(KERN_DEBUG "archos_compass_init: hardware_rev (%i) >= nrev (%i)\n",
			hardware_rev, compass_cfg->nrev);
		return -ENODEV;
	}

	compass_gpio = compass_cfg->rev[hardware_rev];
