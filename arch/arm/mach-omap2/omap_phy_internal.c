...
/* 0 - no utmi, 1 - utmi */
extern int usb_musb_set_phy_interface(int interface);


static struct clk *phyclk, *clk48m, *clk32k;
static void __iomem *ctrl_base;
static void __iomem *scm_base;
...
int omap4430_phy_init(struct device *dev)
{
	...
	scm_base = ioremap(OMAP443X_SCM_BASE, SZ_1K);
	if (!scm_base) {
		dev_err(dev, "scm module ioremap failed\n");
		iounmap(hsotg_base);
		return -ENOMEM;
	}

	ctrl_base = ioremap(OMAP443X_CTRL_BASE, SZ_1K);
	if (!ctrl_base) {
	...
	/* Power down the phy */
	__raw_writel(PHY_PD, scm_base + CONTROL_DEV_CONF);
...
int omap4_charger_detect(void)
{
	...
	default:
		...
		charger = POWER_SUPPLY_TYPE_USB_CDP; // We still allow for 500mA current here
	}
	omap4430_phy_power(NULL, 0, 0);
	usb_musb_set_phy_interface(1);
...
#endif
		/* power on the phy */
		if (__raw_readl(scm_base + CONTROL_DEV_CONF) & PHY_PD) {
			__raw_writel(~PHY_PD, scm_base + CONTROL_DEV_CONF);
			msleep(200);
		}
		if (ID) {			
			/* enable VBUS valid, IDDIG groung */
			__raw_writel(AVALID | VBUSVALID, scm_base +
							USBOTGHS_CONTROL);
		} else {
			...
			__raw_writel(IDDIG | AVALID | VBUSVALID,
						scm_base + USBOTGHS_CONTROL);			
		}
	} else {
		/* Enable session END and IDIG to high impedence. */
		__raw_writel(SESSEND | IDDIG, scm_base +
					USBOTGHS_CONTROL);
		/* Disable the clocks */
