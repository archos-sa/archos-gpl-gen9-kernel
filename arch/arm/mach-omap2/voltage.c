...
static void __init omap3_init_voltagecontroller(void)
{
	...
	if (!vdd_info[impu].pmic || !vdd_info[icore].pmic) {
		// 'omap_voltage_register_pmic not' called before!!!
		pr_warning("pmic not registered. Check board specific file.\n");
		return;
	}
...
static void __init omap4_init_voltagecontroller(void)
{
	...
	if (!vdd_info[impu].pmic || !vdd_info[icore].pmic || !vdd_info[iiva].pmic) {
		// 'omap_voltage_register_pmic not' called before!!!
		pr_warning("pmic not registered. Check board specific file.\n");
		return;
	}
	...
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_CORE_RET_SLEEP_OFFSET,
		...
		(0x9 << OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
		(0x9 << OMAP4430_RAMP_UP_COUNT_SHIFT));
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_IVA_RET_SLEEP_OFFSET,
		(0x0 << OMAP4430_RAMP_DOWN_PRESCAL_SHIFT) |
		(0x0 << OMAP4430_RAMP_UP_PRESCAL_SHIFT) |
		(0x11<< OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
		(0x11 << OMAP4430_RAMP_UP_COUNT_SHIFT));
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_MPU_RET_SLEEP_OFFSET,
		(0x0 << OMAP4430_RAMP_DOWN_PRESCAL_SHIFT) |
		(0x0 << OMAP4430_RAMP_UP_PRESCAL_SHIFT) |
		(0x15 << OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
		(0x15 << OMAP4430_RAMP_UP_COUNT_SHIFT));
	...
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_CORE_OFF_OFFSET,
		...
		(0x4 << OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
		(0x13 << OMAP4430_RAMP_UP_COUNT_SHIFT));
	...
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_WARMRESET_OFFSET,		
		(0x3F << OMAP4430_STABLE_COUNT_SHIFT) |		
...
static void __init vdd_data_configure(struct omap_vdd_info *vdd)
{
	...
	(void) debugfs_create_file("vp_errorgain", S_IRUGO | S_IWUG,
				vdd_debug,
				&(vdd->vp_reg.vpconfig_errorgain),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemin", S_IRUGO | S_IWUG,
				vdd_debug,
				&(vdd->vp_reg.vstepmin_smpswaittimemin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmin", S_IRUGO | S_IWUG, vdd_debug,
				&(vdd->vp_reg.vstepmin_stepmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemax", S_IRUGO | S_IWUG,
				vdd_debug,
				&(vdd->vp_reg.vstepmax_smpswaittimemax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmax", S_IRUGO | S_IWUG, vdd_debug,
				&(vdd->vp_reg.vstepmax_stepmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmax", S_IRUGO | S_IWUG, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmin", S_IRUGO | S_IWUG, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_timeout", S_IRUGO | S_IWUG, vdd_debug,
				&(vdd->vp_reg.vlimitto_timeout),
				&vp_debug_fops);
	...
	if (cpu_is_omap44xx() && !strcmp("vdd_iva", name))
		(void) debugfs_create_u8("fbb_enable", S_IRUGO | S_IWUG,
