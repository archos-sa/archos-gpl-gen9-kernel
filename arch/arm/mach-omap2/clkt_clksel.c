				...
				u32 *new_div)
{
	...
	*new_div = 1;
	...
	for (clkr = clks->rates; clkr->div; clkr++) {
		...
		/* XXX don't limit the precision to 1MHz if target rate
		 *     is below 50MHz. Otherwise the loop exits early
		 *     even if a perfect divider could be found.
		 */
		if (target_rate < 50000000)
			found = (test_rate / 100000) <= (target_rate / 100000);
		else
			found = (test_rate / 1000000) <= (target_rate / 1000000);
