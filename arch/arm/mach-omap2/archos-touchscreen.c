...
static int ts_pwron = UNUSED_GPIO;
static int ts_irq = UNUSED_GPIO; 
...
static int ads7846_vaux_control(int vaux_cntrl)
{
	...
	if (vaux_cntrl == VAUX_ENABLE) {
		gpio_set_value( ts_pwron, enable);
...
static struct omap2_mcspi_platform_config tsc2046_mcspi = {
...
int __init ads7846_dev_init(void)
{
	...
	/* might be NULL */
	if (tsp_cfg == NULL) {
		printk( "ads7846_dev_init: no board configuration found\n");
		return -ENODEV;
	}
	if ( hardware_rev >= tsp_cfg->nrev ) {
		printk( "ads7846_dev_init: hardware_rev (%i) >= nrev (%i)\n",
	...
	tsc2046_config.swap_xy = tsp_cfg->rev[hardware_rev].inversion_flags;
...
*/
	printk( "ads7846_dev_init: irq_gpio %i, pwr_gpio %i\n",
			ts_irq, ts_pwron);

	archos_gpio_init_output(ts_pwron, "ts_pwron");
	tsc2046_config.gpio_pendown = ts_irq;
//	ads7846_vaux_control( VAUX_DISABLE );
	ads7846_vaux_control( VAUX_ENABLE );

	archos_gpio_init_input(ts_irq, "ts_irq");
//	gpio_set_debounce(ts_irq, 310);

	/* fix spi irq gio nb */
	ts_spi_board_info[0].irq = OMAP_GPIO_IRQ(ts_irq);
	spi_register_board_info(ts_spi_board_info,ARRAY_SIZE(ts_spi_board_info));	// 1);
