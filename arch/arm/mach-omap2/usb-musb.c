...
static struct musb_hdrc_config musb_config = {
	...
	&& (defined (CONFIG_USB_ANDROID) || defined (CONFIG_USB_ANDROID_MODULE))
...
/*
 * for freeing the usb interface to make charger detection 
 */

#define OTG_INTERFSEL		(0x40c)
/* 0 - no utmi, 1 - utmi */
int usb_musb_set_phy_interface(int interface)
{
	struct omap_hwmod *oh = oh_p;
	struct platform_device *pdev = &oh->od->pdev;
	struct resource	*iomem;
	void __iomem	*base;
...
#ifdef CONFIG_PM_RUNTIME
	if (oh->od->_state != OMAP_DEVICE_STATE_ENABLED) {
		int ret = omap_device_enable(pdev);	
		if (ret)	
			return ret;
		device_enabled = 1;
	...
	if (interface)
		...
		v = 0x00000001;
	__raw_writel(v, base + OTG_INTERFSEL);
