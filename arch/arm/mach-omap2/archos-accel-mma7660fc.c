/*
 * MMA7660FC Accelerometer board configuration
 */
...
int __init archos_accel_mma7660fc_init(struct mma7660fc_pdata *pdata)
{
	...
	if (accel_gpio.accel_int1 != -1)
		pdata->irq = gpio_to_irq(accel_gpio.accel_int1);
	else
		pdata->irq = -1;
