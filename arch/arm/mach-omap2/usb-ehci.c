...
struct uhhtll_hcd_omap {
	...
	struct omap_hwmod		*tll_hwmod;
	struct omap_hwmod		*ohci_hwmod;
	struct omap_hwmod		*ehci_hwmod;
};
...
#ifdef CONFIG_PM
/* hack for checking 3G stick connection state. */
static int usbhs_is_ehciport0_connected(struct uhhtll_hcd_omap *omap);
int is_ehci_port0_connected(void)
{
	struct uhhtll_hcd_omap *omap = &uhhtll;
	return usbhs_is_ehciport0_connected(omap);
}
#endif
...
static int uhhtll_hcd_omap_suspend(struct platform_device *pdev, pm_message_t state)
{
       struct uhhtll_hcd_omap *omap = &uhhtll;

       if (omap->platdata.deep_bus_suspend) {
               int i;
               usbhs_ehci_io_wakeup(omap->platdata.port_mode, 0);

               for (i = 0 ; i < OMAP3_HS_USB_PORTS ; i++) {
                       if (omap->platdata.regulator[i])
                               regulator_disable(omap->platdata.regulator[i]);
                       if (omap->platdata.regulator_optional[i])
                               regulator_disable(omap->platdata.regulator_optional[i]);
               }
       }
...
static int uhhtll_hcd_omap_resume(struct platform_device *pdev)
{
       struct uhhtll_hcd_omap *omap = &uhhtll;
       ...
       if (omap->platdata.deep_bus_suspend) {
               ...
               for (i = 0 ; i < OMAP3_HS_USB_PORTS ; i++) {
                       if (omap->platdata.regulator[i])
                               regulator_enable(omap->platdata.regulator[i]);
                       if (omap->platdata.regulator_optional[i])
                               regulator_enable(omap->platdata.regulator_optional[i]);
               }

               usbhs_ehci_io_wakeup(omap->platdata.port_mode, 1);
       }
       return 0;
}
...
static struct platform_driver uhhtll_hcd_omap_driver = {
	...
	.suspend		= uhhtll_hcd_omap_suspend,
	.resume			= uhhtll_hcd_omap_resume,
	.driver = {
		.name		= "uhhtll-omap",
	}
};
...
static int is_usb_phy_mode(enum usbhs_omap3_port_mode port_mode) 
{
	switch (port_mode) {
	default:
		return 0;
...
static int usbhs_enable(struct uhhtll_hcd_omap *omap, int do_init)
{
	...
	dev_dbg(&omap->pdev->dev, "Enable USBHS - count=%i - init=%i\n", omap->count, do_init);
	...
	dev_dbg(&omap->pdev->dev, "Switching the regulators ON\n");
	/* get ehci regulator and enable */
	for (i = 0 ; i < OMAP3_HS_USB_PORTS ; i++) {
		if (!is_usb_phy_mode(pdata->port_mode[i])) {
		...
		if (IS_ERR(pdata->regulator[i])) {
			...
			ret = regulator_enable(pdata->regulator[i]);
			if (IS_ERR_VALUE(ret))
				dev_err(&omap->pdev->dev, "error %i enabling "
						"regulator for hsusb%i\n", ret, i);
		}

		snprintf(supply, sizeof(supply), "hsusb_opt%d", i);
		if (!pdata->regulator_optional[i])
			pdata->regulator_optional[i] = regulator_get(&omap->pdev->dev, supply);
		if (IS_ERR(pdata->regulator_optional[i])) {
			pdata->regulator_optional[i] = NULL;
			dev_err(&omap->pdev->dev,
			"failed to get ehci port%d regulator\n", i);
		} else {
			ret = regulator_enable(pdata->regulator_optional[i]);
			if (IS_ERR_VALUE(ret))
				dev_err(&omap->pdev->dev, "error %i enabling "
						"optional regulator for hsusb%i\n", ret, i);
		}
	}

	dev_dbg(&omap->pdev->dev, "Enabling the platform bus\n");
	if (pdata->platform_bus_enable)
		pdata->platform_bus_enable();
	...
	if (cpu_is_omap44xx()) {
		...
		reg |= OMAP_UHH_SYSCONFIG_SWIDLMODE_SET; 	//Smart idle - enable async wakeup

		reg &= OMAP_UHH_SYSCONFIG_STDYMODE_RESET;
		//reg |= OMAP_UHH_SYSCONFIG_SSTDYMODE_SET;
		reg |= OMAP_UHH_SYSCONFIG_SWSTDMODE_SET;	//Smart standby - enable async wakeup
		...
		dev_dbg(&omap->pdev->dev,
			"UHH setup will be done using, uhh_hostconfig=%x\n", reg);
		...
		if ((pdata->port_mode[0] == OMAP_EHCI_PORT_MODE_TLL) ||
			...
			/* Enable TLL Interrupts */
			uhhtll_omap_write(omap->tll_base, OMAP_USBTLL_IRQENABLE,  3);
		}
...
end_init:
	...
	if (cpu_is_omap44xx()) {
		 ...
		 * is properly updated
		 *
		 * This should start the 48 and 12 MHz clocks even when the ohci controller is not loaded
		 */
...
ok_end:
	...
	if (do_stop) {
		dev_dbg(&omap->pdev->dev, "Disabling platform bus\n");
		if (pdata->platform_bus_disable)
			pdata->platform_bus_disable();

		dev_dbg(&omap->pdev->dev, "Switching usb regulators OFF\n");
		for (i = 0 ; i < OMAP3_HS_USB_PORTS ; i++) {
			if (pdata->regulator[i] != NULL) {
				regulator_disable(pdata->regulator[i]);
#if 0
	 // non freeing saves more current :-)
				regulator_put(pdata->regulator[i]);
				pdata->regulator[i] = NULL;
#endif
			}

			if (pdata->regulator_optional[i] != NULL) {
				regulator_disable(pdata->regulator_optional[i]);
#if 0
	 // non freeing saves more current :-)
				regulator_put(pdata->regulator_optional[i]);
				pdata->regulator_optional[i] = NULL;
#endif
			}
		}		
...
static int usbhs_ehci_physuspend(struct uhhtll_hcd_omap *omap)
{
	enum usbhs_omap3_port_mode *portmode = &(omap->platdata.port_mode[0]);
	...
	if (portmode[0] == OMAP_EHCI_PORT_MODE_PHY) {
		reg = ULPI_FUNC_CTRL_SUSPENDM
				/* FUNCTION_CTRL_CLR register */
				| (ULPI_CLR(ULPI_FUNC_CTRL) << EHCI_INSNREG05_ULPI_REGADD_SHIFT)
				/* Write */
		...
		uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);

		/* Wait for ULPI access completion */
		while ((uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI)
				& (1 << EHCI_INSNREG05_ULPI_CONTROL_SHIFT))) {
			cpu_relax();

			if (time_after(jiffies, timeout)) {
				dev_dbg(&omap->pdev->dev, "PHY suspend operation timed out\n");
				return -1;
			}
		}
	}
...
#define GPIO_USBB1_ULPITLL_STP	85
#define GPIO_USBB1_ULPITLL_DIR	86

static int usbhs_ehci_phywakeup(struct uhhtll_hcd_omap *omap)
{
       enum usbhs_omap3_port_mode *portmode = &(omap->platdata.port_mode[0]);
	...
	if (portmode[0] == OMAP_EHCI_PORT_MODE_PHY) {
		...
		if (gpio_request(GPIO_USBB1_ULPITLL_DIR, "phy_dir")) {
			gpio_free(GPIO_USBB1_ULPITLL_STP);
			return -ENODEV;
		}

		omap_mux_init_signal("usbb1_ulpitll_stp.gpio_85", OMAP_PIN_INPUT | OMAP_PIN_OUTPUT);
		gpio_export(GPIO_USBB1_ULPITLL_DIR, false);
		gpio_direction_output(GPIO_USBB1_ULPITLL_STP, 0);

		omap_mux_init_signal("usbb1_ulpitll_dir.gpio_86", OMAP_PIN_INPUT | OMAP_PIN_OUTPUT);
		gpio_export(GPIO_USBB1_ULPITLL_DIR, false);
		gpio_direction_output(GPIO_USBB1_ULPITLL_DIR, 1);

		udelay(10);
		gpio_set_value(GPIO_USBB1_ULPITLL_STP, 1); /* STP */
		gpio_set_value(GPIO_USBB1_ULPITLL_DIR, 1); /* DIR */

		udelay(100);

		gpio_set_value(GPIO_USBB1_ULPITLL_STP, 1); /* STP */
		gpio_set_value(GPIO_USBB1_ULPITLL_DIR, 0); /* DIR */

		udelay(500);

		gpio_set_value(GPIO_USBB1_ULPITLL_STP, 0); /* STP */
		gpio_set_value(GPIO_USBB1_ULPITLL_DIR, 0); /* DIR */

		udelay(100);

		gpio_free(GPIO_USBB1_ULPITLL_STP);
		gpio_free(GPIO_USBB1_ULPITLL_DIR);

		omap_mux_init_signal("usbb1_ulpiphy_stp", OMAP_PIN_OUTPUT);
		omap_mux_init_signal("usbb1_ulpiphy_dir", OMAP_PIN_INPUT_PULLDOWN);
       }
...
static void usbhs_ehci_phyactive(struct uhhtll_hcd_omap *omap)
{
	...
	/* Read current EHCI_INSNREG05 */
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "EHCI_INSREG05 0x%X\n", reg);

	/* Send read command to function control register */
	reg = 0x81C40000;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);

	while (uhhtll_omap_read(omap->ehci_res.regs,
			EHCI_INSNREG05_ULPI) & (1 << 31)) {
		cpu_relax();
	...
	/* Set new function register values */
	timeout = jiffies + msecs_to_jiffies(1000);
	reg = 0x81840045;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);
	...
	/* Read USB Interrupt rising status */
	timeout = jiffies + msecs_to_jiffies(1000);
	reg = 0x81CD0000;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);
	...
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "USB interrupt rising = %X\n", (0xFF & reg));

	/* Read USB Interrupt falling status */
	timeout = jiffies + msecs_to_jiffies(1000);
	reg = 0x81D00000;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);
	...
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "USB interrupt falling = %X\n", (0xFF & reg));
	...
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "USB interrupt status = %X\n", (0xFF & reg));
	
	/* Set USB Interface status */
	timeout = jiffies + msecs_to_jiffies(1000);
	reg = 0x81870010;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);
	...
	udelay(100);	
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "USB interface status = %X\n", (0xFF & reg));
	...
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "USB debug status = %X\n", (0xFF & reg));

	// Perform a write - read test on the scratch register
	timeout = jiffies + msecs_to_jiffies(1000);
	reg = 0x819600CA;
	uhhtll_omap_write(omap->ehci_res.regs, EHCI_INSNREG05_ULPI, reg);
	udelay(100);
	...
	udelay(100);
	reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_INSNREG05_ULPI);
	dev_err(&omap->pdev->dev, "Scratch register = 0x%X - should be 0xCA\n", (0xFF & reg));
...
static int uhhtll_drv_hibernate(enum driver_type drvtype) {
	...
	if (down_interruptible(&omap->mutex))
		...
		omap->uhh_hwmod->flags = (HWMOD_SWSUP_SIDLE |
					HWMOD_SWSUP_MSTANDBY);
		omap->tll_hwmod->flags = (HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		omap->ehci_hwmod->flags = (HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		omap->ohci_hwmod->flags = (HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		...
		if (!omap->usbhost_hs_fck) {
			omap->usbhost_hs_fck = clk_get(NULL, "usb_host_hs_fck");
			if (!omap->usbhost_hs_fck)
				pr_err("Could not get usb_host_ck\n");
		}
		if (!omap->usbhost_ick) {
			omap->usbhost_ick = clk_get(NULL, "usb_tll_hs_ick");
			if (!omap->usbhost_ick)
		...
		/* Enable the EHCI config flag - will route all ports to the ehci controller */
		reg  = uhhtll_omap_read(omap->ehci_res.regs, EHCI_CONFIGFLAG);
		reg |= EHCI_CONFIG_ENABLE;
		uhhtll_omap_write(omap->ehci_res.regs, EHCI_CONFIGFLAG, reg);

		// Send UHH controller in stdby
		dev_err(&omap->pdev->dev,"Deactivate APP_START_CLK\n");
		reg = uhhtll_omap_read(omap->uhh_base, OMAP_UHH_HOSTCONFIG);
		reg &= (0x7FFFFFFF);
		uhhtll_omap_write(omap->uhh_base, OMAP_UHH_HOSTCONFIG, reg);
		jiffies + msecs_to_jiffies(1000);
		while (((reg = uhhtll_omap_read(omap->uhh_base, OMAP_UHH_HOSTCONFIG)) & (1 << 31))) {
			cpu_relax();	// wait for write completion
			if (time_after(jiffies, timeout)) {
				dev_err(&omap->pdev->dev,
				"%s: !APP_START_CLK operation timed out.. \n", __func__);
				break;
			}		
		}
		dev_err(&omap->pdev->dev,"Sending UHH controller in stdby\n");
		reg = uhhtll_omap_read(omap->uhh_base, OMAP_UHH_SYSCONFIG);
//		reg &= OMAP_UHH_SYSCONFIG_STDYMODE_RESET;
		reg = 0;
//		reg &= OMAP_UHH_SYSCONFIG_IDLEMODE_RESET;
		uhhtll_omap_write(omap->uhh_base, OMAP_UHH_SYSCONFIG, reg);
		jiffies + msecs_to_jiffies(1000);
		while (((reg = uhhtll_omap_read(omap->uhh_base, OMAP_UHH_SYSCONFIG)) & (1 << 31))) {
			cpu_relax();	// wait for write completion
			if (time_after(jiffies, timeout)) {
				dev_err(&omap->pdev->dev,
				"%s: Stdby operation timed out.. \n", __func__);
				break;
			}		
		}

		dev_err(&omap->pdev->dev,"Sending OHCI controller in stdby\n");
		// Send OHCI controller in stdby
		reg = uhhtll_omap_read(omap->ohci_res.regs, OHCI_HCCONTROL);
		reg &= OHCI_HCCONTROL_RESET;
		reg |= OHCI_HCCONTROL_SUSPEND;
		uhhtll_omap_write(omap->ohci_res.regs, OHCI_HCCONTROL, reg);
		...
		// Enable clock disable on UTMI and ULPI outputs
		for (i = 0; i < OMAP4_TLL_CHANNEL_COUNT; i++) {
			reg = uhhtll_omap_read(omap->tll_base,
				OMAP_TLL_CHANNEL_CONF(i));
			reg |= OMAP_TLL_CHANNEL_CONF_ULPI_ULPIAUTOIDLE | OMAP_TLL_CHANNEL_CONF_UTMIAUTOIDLE;
			uhhtll_omap_write(omap->tll_base, OMAP_TLL_CHANNEL_CONF(i), reg);
		}

		// Send TLL module in standby
		reg = uhhtll_omap_read(omap->tll_base, OMAP_USBTLL_SYSCONFIG);
		reg = 0x1;
		uhhtll_omap_write(omap->tll_base,
				OMAP_USBTLL_SYSCONFIG,
				reg);
		...
		// Disable the Phy
		dev_err(&omap->pdev->dev, "Disabling platform bus\n");
		if (pdata->platform_bus_disable)
			pdata->platform_bus_disable();
...
*/
		...
		/* set the hwmod to smart idle */
		omap->tll_hwmod->flags &= ~(HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		omap->uhh_hwmod->flags &= ~(HWMOD_SWSUP_SIDLE |	HWMOD_SWSUP_MSTANDBY);
		//omap->ohci_hwmod->flags &= ~(HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		omap->ehci_hwmod->flags &= ~(HWMOD_SWSUP_SIDLE | HWMOD_SWSUP_MSTANDBY);
		...
		// Wake up the EHCI controller	
		dev_err(&omap->pdev->dev, "Waking up EHCI controller\n");
		reg = uhhtll_omap_read(omap->uhh_base, OMAP_UHH_HOSTCONFIG);
		reg |= OMAP4_UHH_HOSTCONFIG_APP_START_CLK;
		uhhtll_omap_write(omap->uhh_base, OMAP_UHH_HOSTCONFIG, reg);	
		timeout = jiffies + msecs_to_jiffies(1000);
...
void __init usb_uhhtll_init(const struct usbhs_omap_platform_data *pdata)
{
	...
	omap->uhh_hwmod = oh[0];
	omap->tll_hwmod = oh[1];
	omap->ohci_hwmod = ohci_oh;
	omap->ehci_hwmod = ehci_oh;
