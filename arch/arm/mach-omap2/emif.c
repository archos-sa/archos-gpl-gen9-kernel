 ...
 * Sets the shadow registers using pre-calculated register values
...
static irqreturn_t emif_interrupt_handler(int irq, void *dev_id)
{
	...
	log = __raw_readl(base + OMAP44XX_EMIF_OCP_ERR_LOG);
	...
	if (sys & OMAP44XX_REG_ERR_SYS_MASK)
		pr_err("EMIF: Access error from EMIF%d SYS port - %x  log %x",
			emif_nr, sys, log);

	if (ll & OMAP44XX_REG_ERR_LL_MASK)
		pr_err("EMIF Error: Access error from EMIF%d LL port - %x  log %x",
			emif_nr, ll, log);
