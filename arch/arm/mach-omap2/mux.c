				 ...
				 struct omap_board_mux *board_mux)
{
	while (board_mux->reg_offset != OMAP_MUX_TERMINATOR) {
		...
		u16 old_mode = omap_mux_read(partition,
					 board_mux->reg_offset);
		u16 mux_mode = board_mux->value;
		
		pr_debug("%s: Setting mux offset 0x%04x "
			 "0x%04x -> 0x%04x\n",
			 __func__, board_mux->reg_offset,
			old_mode, mux_mode);
...
u16 omap_mux_read_signal(const char *muxname)
{
	...
	pr_debug("%s: Could not read signal %s\n", __func__, muxname);
