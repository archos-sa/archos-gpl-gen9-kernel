...
static void omap_uart_mdr1_errataset(struct uart_omap_port *up, u8 mdr1_val,
		u8 fcr_val)
{
			...
			struct ktermios *old)
{
	...
	omap_uart_mdr1_errataset(up, OMAP_MDR1_DISABLE, up->fcr);
	...
	if (baud > 230400 && baud != 3000000 && baud != 1000000)
		omap_uart_mdr1_errataset(up, OMAP_MDR1_MODE13X, up->fcr);
	else
		omap_uart_mdr1_errataset(up, OMAP_MDR1_MODE16X, up->fcr);
