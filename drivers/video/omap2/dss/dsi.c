		...
		struct dsi_clock_info *cinfo)
{
	...
	if (cinfo->use_dss2_fck) {
		if (cpu_is_omap44xx()) {
			cinfo->clkin = dss_clk_get_rate(DSS_CLK_ICK);
		...
		struct dispc_clock_info *dispc_cinfo)
{
	...
	if (cpu_is_omap44xx())
		dss_clk_fck2 = dss_clk_get_rate(DSS_CLK_ICK);
	else
	...
	for (cur.regn = 1; cur.regn < REGN_MAX; ++cur.regn) {
		...
		for (cur.regm = 1; cur.regm < REGM_MAX; ++cur.regm) {
			...
			for (cur.regm_dispc = 1; cur.regm_dispc < REGM_DISPC_MAX;
				...
				cur.dsi_pll_dispc_fclk = cur.clkin4ddr / cur.regm_dispc;

				/* this will narrow down the search a bit,
				 * but still give pixclocks below what was
				 * requested */
				if (cur.dsi_pll_dispc_fclk  <  ((cpu_is_omap44xx() ? 2 : 1) * req_pck))
					break;

				if (cur.dsi_pll_dispc_fclk > DISPC_MAX_FCK)
					continue;

				if (min_fck_per_pck && cur.dsi_pll_dispc_fclk <
...
//		l = FLD_MOD(l, 3, 22, 21);	/* DSI_REF_SEL */
	...
	dsi_pll_power(ix, DSI_PLL_POWER_ON_ALL);
