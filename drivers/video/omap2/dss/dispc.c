...
void dispc_load_gamma_table(int table_num, u32 *gamma_tab)
{
	int i;

	enable_clocks(1);
	for (i = 0; i< 256; i++) {
		dispc_write_reg(DISPC_GAMMA_TABLE(table_num), gamma_tab[i]);
	}
	enable_clocks(0);
}
...
void dispc_set_gamma_table(enum omap_channel channel, u32 *gamma_table)
{
	...
	gamma_tab = kmalloc(3 * 256 * 4, GFP_KERNEL);

	enable_clocks(1);
	dispc_write_reg(DISPC_GFX_TABLE_BA, ((u32)gamma_tab & 0xffffff00) );

	if (channel == OMAP_DSS_CHANNEL_LCD2)
		dispc_load_gamma_table(1, gamma_table);
	else if (channel == OMAP_DSS_CHANNEL_LCD)
		dispc_load_gamma_table(0, gamma_table);
	else if (channel == OMAP_DSS_CHANNEL_DIGIT)
		dispc_load_gamma_table(2, gamma_table);
	...
	while (1) {
		...
		*three_tap = false;
		...
		if (!cpu_is_omap44xx() && fclk > fclk_max) {
			/* only omap4 can decimate, on omap2/3 we want to use 5tap 
			 * if 3-tap demands a too high clock freq, so disable forcing
			 * 3-tap and try with 5-tap 
			 */
			DSSDBG("3-tap clk too high %d > %d, try 5-tap\n", (int)fclk, (int)fclk_max );
			*three_tap = false;
		}
		fclk5 = *three_tap ? 0 :
...
void dispc_set_cpr_mode(enum omap_channel channel, int enable, int full_range)
{
	enable_clocks(1);
	if (channel == OMAP_DSS_CHANNEL_LCD2) {

		REG_FLD_MOD(DISPC_CONFIG2, full_range ? 1 : 0, 25, 25);

		dispc_write_reg(DISPC_CPR2_COEF_B, 0x0100l);
		dispc_write_reg(DISPC_CPR2_COEF_G, 0x80000l);
		dispc_write_reg(DISPC_CPR2_COEF_R, 0x40000000l);
		REG_FLD_MOD(DISPC_CONFIG2, enable ? 1 : 0, 15, 15);
	}
	enable_clocks(0);
}
...
EXPORT_SYMBOL(dispc_is_vsync_fake);
