		...
		unsigned long pck_req, unsigned long *pck)
{
	...
	if (!cpu_is_omap44xx()) {
		...
		// !!! based on a dss clk of 38400000
		dsi_cinfo.regn = 17;
		dsi_cinfo.regm = 150;
		...
		bool is_tft, unsigned long pck_req, unsigned long *pck)
{
	...
	DSSDBG("DPI clk source is DISPC %d\n", (int) dss_clk_get_rate(DSS_CLK_FCK1));
...
#ifdef CONFIG_OMAP2_DSS_USE_DSI_PLL
	if ( ! dssdev->phy.dpi.force_dispc_clk ) {
...
static void dpi_basic_init(struct omap_dss_device *dssdev)
{
	...
	dispc_set_dither_mode(dssdev->channel, dssdev->phy.dpi.dither);

	if (dssdev->phy.dpi.gamma_correction){
		dispc_set_gamma_table(dssdev->channel, dssdev->phy.dpi.gamma_correction);
...
void dpi_get_timings(struct omap_dss_device *dssdev,
			struct omap_video_timings *timings)
{
	*timings = dssdev->panel.timings;
}
EXPORT_SYMBOL(dpi_get_timings);
