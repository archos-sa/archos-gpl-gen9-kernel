					...
					struct omap_video_timings *timings)
{
	...
	case HDMI_EDID_DTD_TAG_MONITOR_LIMITS:
	{
        ...
        for (i = OMAP_HDMI_TIMINGS_VESA_START;
			...
			hscan /= 1000;
			pixclock /= 1000000;
			DSSDBG("debug only pixclock=%d, hscan=%d, hz=%d\n",
					pixclock, hscan, hz);
		...
		if (max_area)
			DSSDBG( "found best resolution: %dx%d\n",
								...
								int *offset)
{
	...
	while (i < (0x80 + disp)) {
		current_byte = edid[i];
		DSSDBG( "i = %x cur_byte = %x (cur_byte & EX_DATABLOCK_TAG_MASK) = %d\n",
			i, current_byte,
			(current_byte & HDMI_EDID_EX_DATABLOCK_TAG_MASK));
		if ((current_byte >> 5)	== datablock) {
			*offset = i;
			DSSDBG( "datablock %d %d\n",
							datablock, *offset);
			return 0;
		} else {
			length = (current_byte &
