 ...
 */
static const int omap443x_adc_to_temp[] = {
...
static int omap443x_adc_to_temp_conversion(int adc_val)
{
	return omap443x_adc_to_temp[adc_val];
}
...
static struct attribute *omap443x_temp_sensor_attributes[] = {
	&sensor_dev_attr_temp1_input.dev_attr.attr,
#ifdef TEMP_DEBUG
	&sensor_dev_attr_debug_user.dev_attr.attr,
#endif
...
static const struct attribute_group omap443x_temp_sensor_group = {
	.attrs = omap443x_temp_sensor_attributes,
};
...
static int __devinit omap_temp_sensor_probe(struct platform_device *pdev)
{
	...
	if(cpu_is_omap446x()) {
		...
		/* OMAP443x does not really benefit from continuous mode, as no IRQ available */
		omap443x_enable_continuous_mode(temp_sensor, 0);
...
MODULE_DESCRIPTION("OMAP44XX temperature sensor Driver");
MODULE_LICENSE("GPL");
