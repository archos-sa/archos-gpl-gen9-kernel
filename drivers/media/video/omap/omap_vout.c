...
intlace:
	...
	if (!list_empty(&vout->dma_queue)) {
		struct videobuf_buffer *nxt = list_entry(vout->dma_queue.next, struct videobuf_buffer, queue);
		if( timeval_compare( &timevalue, &nxt->ts ) < 0 ) {
v4l2_dbg(1, debug, &vout->vid_dev->v4l2_dev, "%8d|%6d  %8d|%6d  wait\r\n",
					   (int)timevalue.tv_sec, (int)timevalue.tv_usec, 
					   (int)nxt->ts.tv_sec, (int)nxt->ts.tv_usec );
			// we are too early for this frame, wait
			spin_unlock_irqrestore(&vout->vbq_lock, flags);
			return;
		}
v4l2_dbg(1, debug, &vout->vid_dev->v4l2_dev, "%8d|%6d  %8d|%6d  OUT \r\n",
					   (int)timevalue.tv_sec, (int)timevalue.tv_usec, 
					   (int)nxt->ts.tv_sec, (int)nxt->ts.tv_usec ); 
	}
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	v4l2_dbg(1, debug, &vout->vid_dev->v4l2_dev, "vidioc_streamon: %d\n", vout->rotation);
...
printk(KERN_INFO "omap_vout_setup_video_bufs (%d) p %08X  v %08X\n", i, (unsigned int)vout->buf_phy_addr[i], (unsigned int)vout->buf_phy_addr[i]);
		if (!vout->buf_virt_addr[i]) {
			numbuffers = i;
			ret = -ENOMEM;
			goto free_buffers;
