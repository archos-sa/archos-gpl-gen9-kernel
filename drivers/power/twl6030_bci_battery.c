...
struct twl6030_bci_device_info {
	...
	unsigned int		usb_is_dc;
	unsigned int		inihibit_charge;
#endif
...
#ifdef CONFIG_MACH_ARCHOS
	if ( di->inihibit_charge ) {
		dev_dbg(di->dev, "charge inhibit, don't enable charging here\n");
		return;
	}
#endif
	if (di->charger_incurrentmA >= 50)
...
static int twl6030battery_current(struct twl6030_bci_device_info *di)
{
	...
	/* Check if auto-calibration is ongoing */
	ret = twl_i2c_read_u8(TWL6030_MODULE_GASGAUGE, &offset_reg, 
								FG_REG_00);
	if (!IS_ERR_VALUE(ret)) {
		if (offset_reg & CC_CAL_EN) {
			dev_dbg(di->dev, "auto-calibration cycle ongoing\n");
			return -EAGAIN;
		}
	}
...
#ifdef CONFIG_MACH_ARCHOS
static struct twl6030_bci_device_info *local_di;

void twl6030_inhibit_usb_charging( int inhibit )
{	
	local_di->inihibit_charge = inhibit;
	if( inhibit )
		twl6030_stop_usb_charger(local_di);
	else
		twl6030_start_usb_charger(local_di);
}

EXPORT_SYMBOL(twl6030_inhibit_usb_charging);
#endif
...
static int __devinit twl6030_bci_battery_probe(struct platform_device *pdev)
{
	...
	// if ((pdata->bootup_limit_currentmA > 0)	&& (cin_limit_preboot != 0xF))	// when current_limiting is wanted and avboot did
	// 	di->charger_incurrentmA = pdata->bootup_limit_currentmA;	// limit to something use the limit in pdata
...
static int twl6030_bci_battery_resume(struct platform_device *pdev)
{
	...
	ret = twl6030battery_temp_setup();
	if (ret)
		dev_dbg(&pdev->dev, " temperature sensing setup failed\n");
