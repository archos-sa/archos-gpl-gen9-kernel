...
#define BULK_BUFFER_SIZE           (4*1024)
...
static void mtp_complete_out_file(struct usb_ep *ep, struct usb_request *req)
{
...
static void send_file_work(struct work_struct *data) {
	...
	DBG(cdev, "Will transfer file sized %lld bytes\n",count);
	if (count > 0xFFFFFFFF)
		count = 0xFFFFFFFF;
	...
	while (count > 0 || sendZLP) {
		...
		partial_page = file_offset & (PAGE_CACHE_SIZE - 1); // try to stay within page
		if (partial_page > 0)
			xfer = (xfer > (unsigned int) PAGE_CACHE_SIZE - partial_page)?
...
static void receive_file_work(struct work_struct *data)
{
	...
	struct usb_composite_dev *cdev = dev->cdev;
	struct usb_request *read_req[RX_REQ_MAX], *write_req[RX_REQ_MAX];
	struct file *filp;
	...
	int ret, cur_buf = 0;
	int r = 0;
	unsigned int partial_page=0;
	unsigned int rd = 0;		//reads done
	unsigned int wd = 0;		//writes done
	unsigned int rip = 0;		// reads in progress
	...
	while ((count > 0 || write_req[wd])) {
		while ((count > 0) && (read_req[cur_buf] == NULL) && (!write_req[wd])) {
			/* queue a request */
			//DBG(cdev, "Queueing request %i\n", cur_buf);
			read_req[cur_buf] = dev->rx_req[cur_buf];

			read_req[cur_buf]->length = (count > BULK_BUFFER_SIZE
					? BULK_BUFFER_SIZE : count);
			partial_page = file_offset & (PAGE_CACHE_SIZE - 1); // try to stay within page
			if (partial_page > 0)
				read_req[cur_buf]->length =
					(read_req[cur_buf]->length > (unsigned int) PAGE_CACHE_SIZE - partial_page)?
					(unsigned int) PAGE_CACHE_SIZE - partial_page:
					read_req[cur_buf]->length;
			read_req[cur_buf]->complete = (void*) mtp_complete_out_file;
			//DBG(cdev, "requesting %d bytes\n", read_req[cur_buf]->length);
			file_offset += read_req[cur_buf]->length;
			ret = usb_ep_queue(dev->ep_out, read_req[cur_buf], GFP_KERNEL);
			//spin_lock_irq(&dev->lock);
			dev->rx_done = 0;
			if (ret < 0) {
				r = -EIO;
		...
		if ((dev->state == STATE_ERROR) && (r ==-EIO))
			break;

		//if ((!write_req[wd]) && (rip>0))
		//	ret = wait_event_interruptible(dev->read_wq,
		//		dev->rx_done || dev->state != STATE_BUSY);

		while (write_req[wd]) {
			//DBG(cdev, "Writing request %i\n",wd);
			//DBG(cdev, "rx %p %d\n", write_req[wd], write_req[wd]->actual);
			ret = vfs_write(filp, write_req[wd]->buf, write_req[wd]->actual,
				&offset);
			ret = write_req[wd]->actual;
			//DBG(cdev, "vfs_write %d\n", ret);
			if (ret != write_req[wd]->actual) {
				r = -EIO;
				//spin_lock_irq(&dev->lock);
				dev->state = STATE_ERROR;
				//spin_unlock_irq(&dev->lock);
				...
				{
					rip--;
					//DBG(cdev, "dequeueing packet %i, %i to go\n", rd, rip);
					usb_ep_dequeue(dev->ep_out, read_req[rd]);
					read_req[rd]->complete = (void*) mtp_complete_out;
					read_req[rd]->status = 0;
					rd = (rd + 1) % RX_REQ_MAX;
				}
			}
			write_req[wd] = NULL;
		...
		if (rip>0) {
			...
			if (read_req[rd])
				while ((read_req[rd]->status == 1) && (! write_req[rd]))
				{
					//printk( "Request %i returned\n",rd);
					if (count != 0xFFFFFFFF)
						count -= read_req[rd]->actual;
					write_req[rd] = read_req[rd];
					rip--;
					read_req[rd]->complete = (void*) mtp_complete_out;
					read_req[rd]->status = 0;
					if (read_req[rd]->actual < read_req[rd]->length) {
						/* short packet is used to signal EOF for sizes > 4 gig */
						printk( "got short packet nr. %i - requested %i, got %i\n",
							rd, read_req[rd]->length, read_req[rd]->actual);
						rd = (rd + 1) % RX_REQ_MAX;
						while (rip>0)
						{
							rip--;
