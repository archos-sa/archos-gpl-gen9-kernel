...
printk("STATE_UNKNOWN -> STATE_NONE\n");
		...
		blocking_notifier_call_chain(&xceiv->otg.notifier, USB_EVENT_NONE,
				&charger_type);
...
printk("STATE_NONE -> STATE_GND\n");
			...
			blocking_notifier_call_chain(&xceiv->otg.notifier,
...
static int __devinit archos_usb_xceiv_probe(struct platform_device *pdev)
{
	...
	BLOCKING_INIT_NOTIFIER_HEAD(&xceiv->otg.notifier);
