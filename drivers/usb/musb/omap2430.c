...
void musb_enable_vbus(struct musb *musb)
{
	...
	while (musb_readb(musb->mregs, MUSB_DEVCTL) & 0x80) {
		...
		DBG(1, "devcontrol before vbus=%x\n", musb_readb(musb->mregs,
						 MUSB_DEVCTL));
		printk("devcontrol before vbus=%x\n", musb_readb(musb->mregs,
						 MUSB_DEVCTL));
	}
		...
		unsigned long event, void *unused)
{
	...
	case USB_EVENT_NONE:
		...
		if (data->interface_type == MUSB_INTERFACE_UTMI) {
			...
			val |= ULPI_12PIN;
			musb_writel(musb->mregs, OTG_INTERFSEL, val);
		} else if (data->interface_type == MUSB_INTERFACE_ULPI) {
			if (musb->xceiv->set_vbus)
				otg_set_vbus(musb->xceiv, 0);
		}
...
int __init musb_platform_init(struct musb *musb)
{
	...
	if (cpu_is_omap44xx()) {
		...
		ctrl_base = ioremap(0x480ab000, SZ_1K);
		if (!ctrl_base) {
			dev_err(dev, "ioremap failed\n");
			phymux_base = NULL;
			return -ENOMEM;
