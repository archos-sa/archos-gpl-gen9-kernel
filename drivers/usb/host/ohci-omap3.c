...
static int ohci_omap3_bus_die_silently(struct usb_hcd *hcd) {
	struct device *dev = hcd->self.controller;
	struct uhhtll_apis *uhhtllp = dev->platform_data;
	
	int ret = 0;	
	if (uhhtllp && uhhtllp->suspend)
		ret = uhhtllp->suspend(OMAP_OHCI);
	uhhtllp->suspend = NULL;
	return ret;
}

static int ohci_omap3_bus_dont_resume(struct usb_hcd *hcd){
	struct device *dev = hcd->self.controller;
	struct uhhtll_apis *uhhtllp = dev->platform_data;
	int ret = 0;	
...
#else
	.bus_suspend = 		ohci_omap3_bus_die_silently,
	.bus_resume = 		ohci_omap3_bus_dont_resume,
#endif
#endif
