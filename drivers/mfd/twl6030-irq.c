...
static int twl6030_irq_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		 ...
		 *
		 * Additionally fix of losing disconnect interrupts if
		 * charger fault interrupt signalized during connect
		 */
		
		/* Needs REVIEW. Revert patch for detecting usb disconnect more safe.
	 	   It is more important to detect deticated charger for demo. */
#if 1 
		if (sts.bytes[2] & 0x50)
			sts.bytes[2] |= 0x08;
#else
