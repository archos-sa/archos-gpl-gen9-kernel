	 	...
	 	if (state == 0) {
	 		...
	 		if (*end == ':') {
				...
				if (++(*num_cards) == MMC_BLOCK_NUM_SLICED_CARDS)
	 				return 0;
	 			len = end-start > 9 ? 9 : end-start;
				if (len == 0)
					return -1;
				memcpy(cards[*num_cards].host_slot_name, start, len);
				cards[*num_cards].host_slot_name[len] = '\0';
	 			start = end+1;
	 			state = 2;			
	 		}
		} else if (state == 2) {
...
//printk("parse, host name: %s, slice: %d, size: %llu\n",cards[*num_cards].host_slot_name, cards[*num_cards].num_slices, size);
...
static void mmc_blk_put(struct mmc_blk_data *md)
{
	...
	if (md->usage == 0) {
		...
		for (i=0; i<md->num_disks; i++) {
			int devidx = md->disk[i]->first_minor >> MMC_SHIFT;
...
#ifdef CONFIG_MMC_BLOCK_CARD_SPLIT
	if (num_sliced_cards > 0) {
		sector_t tmp;
		for (i=0; i<num_sliced_cards; i++) {
//printk("host name list: %s\n", sliced_cards[i].host_slot_name);
			if (!strcmp(sliced_cards[i].host_slot_name,card->host->name)) {
	...
	for (i=0; i<initialized; i++) {	
		...
		md->disk[i]->flags = GENHD_FL_EXT_DEVT;
...
static int __init mmc_blk_init(void)
{
	...
	if (parse_split_params(sliced_cards, &num_sliced_cards, (const char*)split_params_string) < 0) {
	       printk(KERN_INFO "mmcblk: no split parameter\n");
#ifdef CONFIG_MMC_BLOCK_CARD_SPLIT_PARAMETER
		if (parse_split_params(sliced_cards, &num_sliced_cards, split_params_default_string) < 0) {
			printk(KERN_ERR "mmcblk: no default parameter\n");
		}
	} 
#endif
#endif 
